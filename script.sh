...
wget -qq $(wget -qO- https://www.minecraft.net/en-us/download/server/bedrock | grep serverBedrockLinux | sed 's| class="btn btn-disabled-outline mt-4 downloadlink" role="button" data-platform="serverBedrockLinux">Download </a>||g' | sed 's|<a href=||g' | sed 's| ||g' | sed 's|"||g') -O /tmp/bds.zip
echo "wget status: $?"
